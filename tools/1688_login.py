# -*- coding: utf-8 -*-
"""1688 登录态录入：打开可见 Chrome，由用户自行扫码后导出 Cookie。"""
import os
import sys
import time
...
PROFILE_DIR = os.path.join(BASE_DIR, '1688_profile')
...
def _logged_in(context):
    names = {c['name'] for c in context.cookies('https://www.1688.com')}
    return bool(names & LOGIN_COOKIES)


def main():
    print('正在打开 1688，请在 Chrome 窗口自行扫码登录（最长等待 8 分钟）...', flush=True)
    with sync_playwright() as p:
        ...
        if not _logged_in(ctx):
            print('未检测到 1688 登录态，未覆盖原 Cookie。', flush=True)
            ctx.close()
            return 2
        value = _cookie_string(ctx)
        with open(COOKIE_FILE, 'w', encoding='utf-8') as f:
            f.write(value)
        print('1688 登录态已保存（%d 个 Cookie 字段）。' % len(ctx.cookies()), flush=True)
        ctx.close()
        return 0
