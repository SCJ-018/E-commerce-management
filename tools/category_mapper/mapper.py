...
CATEGORY_LIST = [
    ...
    '车用电子',
    # 非车品
    '保温杯/水杯', '保温箱/冷藏箱', '内衣/服饰', '安防/探测', '个护/小家电',
    '健身器材',
    # 特殊 + 兜底
    '补差价链接', '其他',
]
...
RULES = {
    ...
    # 保温箱/冷藏箱（2026-09-17 新增，第 26 个品类）
    # 起因：3 条千牛保温箱/冷藏箱（出胜/御车宝）当初被 LLM 塞进「保温杯/水杯」；
    # 该品日均支付 ~147 元（09-15 单日 753 元），远超「小金额杂项」线，单独成品类
    '保温箱/冷藏箱': ['保温箱', '冷藏箱', '保鲜箱', '保温桶', '冷藏桶', '冰桶',
                    '保温包', '冷藏包', '保温袋', '冷藏袋'],
    '内衣/服饰': ['内衣', '文胸', '抹胸', '吊带', '无钢圈', '聚拢', '桑蚕丝'],
    '安防/探测': ['探测仪', '防偷拍', '反偷拍', '探测器', '防窥'],
    '个护/小家电': ['剃须刀', '刮胡刀', '吹风机', '电动牙刷'],
    # 健身器材（2026-09-17 新增，强域爬楼机不再是「其他」）
    # ⚠️ 刻意不放「健身」「运动」这类泛词：会与「运动款脚垫」等标题撞车（≥2 命中→全交 LLM）
    '健身器材': ['爬楼机', '爬坡机', '楼梯机', '登山机', '踏步机', '踏步器',
                '跑步机', '走步机', '椭圆机', '划船机', '动感单车', '哑铃',
                '甩脂机', '健腹轮', '健身器材', '健身器'],
}
...
# 强制归「其他」的关键词：命中即判「其他」，既不走其它规则也不走 LLM（优先级仅次于补差价）
# 用途（用户规则 2026-09-17）：**品类表里确实没有合适品类、且金额很小（日均支付 < 100 元）的杂项**，
#   不为它单开品类，直接归「其他」。
# 现有成员：水杯垫（车内装饰/门槽垫，日均 ~15 元，曾被「水杯」子串误命中到「保温杯/水杯」）
FORCE_OTHER_KEYWORDS = ['水杯垫']
...
EXCLUDE = {
    '后备箱垫': ['脚垫', '地垫'],
    '摆件/装饰': ['香薰', '香水', '香氛', '车载香'],
    # 「充电」这个词太泛：电动牙刷/剃须刀标题里的「充电式」会误命中「车用电子」，
    # 导致 2 个品类同时命中 → 交给 LLM（而「电动牙刷」本来就是 个护/小家电 的强关键词）
    '车用电子': ['电动牙刷', '牙刷', '剃须刀', '刮胡刀', '吹风机'],
    # 「水杯」在「保温杯/水杯」里是强关键词，但「水杯垫」是垫子类，被它子串误命中
    # （2026-09-17：2 条千牛「水杯垫」曾落到 保温杯/水杯）
    '保温杯/水杯': ['水杯垫'],
}
...
def classify_by_rules(title):
    ...
    特例：命中 FORCE_OTHER_KEYWORDS 时直接返回「其他」（确定性，不再交 LLM）。
    """
    tl = title.lower()
    # 补差价链接优先级最高
    if any(k in tl for k in SPECIAL_LINK_KEYWORDS):
        return '补差价链接'
    # 明确「无合适品类」的杂项：直接其他（优先级仅次于补差价）
    if any(k in tl for k in FORCE_OTHER_KEYWORDS):
        return '其他'
    hits = []
...
def reclassify_uncategorized(conn):
    ...
    for key, title in items.items():
        ...
        # ★ 注意：classify_by_rules 返回「其他」= FORCE_OTHER 命中的**确定性结论**（不是「没判出来」），
        #   这里必须当规则结果用；若沿用旧的 `cat != '其他'` 判断，会被再丢给 LLM 判歪。
        if cat:
            rule_hit[key] = cat
        else:
