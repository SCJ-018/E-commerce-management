...
def main():
    ...
    print('=' * 64)
    print(' 保持这个窗口开着就行（可最小化）。关掉窗口 = 停止助手，后台按钮将无人响应。')
    print(' 之后在后台「店铺账号管理 -> 抖店 -> 抖店登录邮箱」点「手动拖滑块」，')
    print(' 这台电脑会自动弹出 Chrome，拖完拼图滑块即可，登录态自动传到服务器。')
    print(' 想开机自动待命：Win+R 输入 shell:startup，把「启动滑块助手.bat」快捷方式丢进去。')
    print('-' * 64)
